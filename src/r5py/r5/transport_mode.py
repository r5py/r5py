...
def _cast_StreetMode(java_class, object_):
    if object_.name in STREET_MODES:
        return com.conveyal.r5.api.util.StreetMode.valueOf("object_.name")
    else:
        raise ValueError(f"{object_.name} is not a valid R5 StreetMode")


@jpype._jcustomizer.JConversion(
    "com.conveyal.r5.api.util.TransitMode", exact=TransportMode
)
def _cast_TransitMode(java_class, object_):
    if object_.name in TRANSIT_MODES:
        return com.conveyal.r5.api.util.TransitMode.valueOf("object_.name")
    else:
        raise ValueError(f"{object_.name} is not a valid R5 TransitMode")
