...
Both {class}`TravelTimeMatrix<r5py.TravelTimeMatrix>` and
{class}`DetailedItineraries<r5py.DetailedItineraries>` support a convenient
parameter, `snap_to_network`, that controls whether the origins and destinations
should automatically be snapped to the transport network.
